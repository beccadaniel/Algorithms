...
public class Indicium {
    ...
    private static Pair isTracePossible(int trace, int matrixSize) {
        int[] firstrow = new int[matrixSize];
        for (int i = 0; i < matrixSize; i++) {
            firstrow[i] = i + 1; // first row
        }
        List<List<Integer>> rowPermute = permute(firstrow);

        for (List<Integer> row: rowPermute) {

            int[][] matrix = getRowMatrices(matrixSize, row);
